#For Figure 5 of section 5.2 of the first paper: this code is about LSA-NROWL2.
...
td.mat_mfi_choose.total.nrow <- dtm/matrix(rep(sqrt(apply(dtm^2,1,sum)),each = ncol(dtm)), ncol = ncol(dtm), by = TRUE)
...
dev.off()
setEPS()
postscript("C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\20Newsgroups fold in\\figure\\20Newsgroups visualization\\F20NewsgroupsLSANROWL2.eps")
par(mar=c(5,6,4,1)+.1)
plot(td.mat_mfi_choose.total.svd.ud[1:sum(index==1),1], td.mat_mfi_choose.total.svd.ud[1:sum(index==1),2], asp = 1, col = "black", cex = 1.8, type = "p", pch = 1, xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd),#xlim = c(-15,0), ylim = c(-20,20), #xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd), 
     main = "LSA-NROWL2",
     xlab = "Dimension 1: 87.500 (3.0%)", ylab = "Dimension 2: 65.226 (2.2%)",cex.lab=1.75, cex.axis = 1.55, cex.main=1.75)
