#For Figure 3 of section 5.2 of the first paper: this code is about LSA-RAW.
...
dev.off()
setEPS()
postscript("C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\BBCNews data 5 fold in\\figure\\BBCNews visualization\\FBBCNewsLSARAW.eps")
par(mar=c(5,6,4,1)+.1)
plot(td.mat_mfi_choose.total.svd.ud[1:sum(index==1),1], td.mat_mfi_choose.total.svd.ud[1:sum(index==1),2], asp = 1, col = "black", cex = 1.8, type = "p", pch = 1, xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd),#xlim = c(-15,0), ylim = c(-20,20), #xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd), 
     main = "LSA-RAW",
     xlab = "Dimension 1: 58,875.070 (6.2%)", ylab = "Dimension 2: 23,488.330 (2.5%)",cex.lab=1.75, cex.axis = 1.55, cex.main=1.75)
