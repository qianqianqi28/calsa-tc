#For Figure 3 of section 5.2 of the first paper: this code is about creating raw document-term matrix for 2225 documents from 5 categories.
...
for (i in 1:length(index)){
  if (grepl('business', index[i])) {
    index[i] <- 1
  } else if (grepl('entertainment', index[i])) {
    index[i] <- 2
  } else if (grepl('politics', index[i])) {
    index[i] <- 3
  } else if (grepl('sport', index[i])) {
    index[i] <- 4
  } else if (grepl('tech', index[i])) {
    index[i] <- 5
  }
}
...
save(dtm, index, file = "C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\BBCNews data 5 fold in\\figure\\BBCNews visualization\\rawdtmBBCnewsvisu.Rdata")
