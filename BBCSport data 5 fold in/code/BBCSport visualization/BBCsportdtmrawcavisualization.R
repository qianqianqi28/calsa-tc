#For Figure 4 of section 5.2 of the first paper: this code is about CA.
...
td.mat_mfi_choose.total <- as.matrix(dtm)
...
k <- 2
td.mat_mfi_choose.total.svd.ud <- as.matrix(td.mat_mfi_choose.total.Drmh%*%td.mat_mfi_choose.total.svd$u[,1:k])%*%as.matrix(diag(td.mat_mfi_choose.total.svd$d[1:k]))
...
setEPS()
postscript("C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\BBCSport data 5 fold in\\figure\\BBCSport visualization\\FBBCSportCA.eps")
par(mar=c(5,6,4,1)+.1)
...
plot(td.mat_mfi_choose.total.svd.ud[1:sum(index==1),1], td.mat_mfi_choose.total.svd.ud[1:sum(index==1),2], asp = 1, col = "black", cex = 1.8, type = "p", pch = 1, xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd),#xlim = c(-15,0), ylim = c(-20,20), #xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd), 
     main = "CA",
     xlab = "Dimension 1: 0.467 (1.4%)", ylab = "Dimension 2: 0.409 (1.3%)",cex.lab=1.75, cex.axis = 1.55, cex.main=1.75)
...
legend("bottomright",cex=1,c("athletics","cricket","football","rugby","tennis"),col=c("black","blue","orange","green", "Violet"),pch = c(1, 3, 23, 0, 8))
dev.off()
