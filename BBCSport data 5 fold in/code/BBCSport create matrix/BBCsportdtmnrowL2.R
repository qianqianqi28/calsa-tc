#For section 5.3 of the first paper: this code is for 737 documents from five categories using five-fold cross validation. Note each time the documents from the training set form a nrowl2 document-term matrix and the documents of validation set are not included in the matrix.
...
save(training_dtm, testing_dtm, training_index, testing_index, file = "C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\BBCSport data 5 fold in\\created data\\created matrix\\nrowL2dtmBBCsport.Rdata")
timeend<-Sys.time()
timeend- timestart
