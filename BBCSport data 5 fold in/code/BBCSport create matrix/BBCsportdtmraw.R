#For section 5.3 of the first paper: this code is for 737 documents from five categories using five-fold cross validation. Note each time the documents from the training set form a raw document-term matrix and the documents of validation set are not included in the matrix.
...
for(n in fold_range){
  ...
  which(apply(training_dtm[[n]], 1, sum) == 0)
  which(apply(testing_dtm[[n]], 1, sum) == 0)
  ...
  for (i in 1:length(training_index[[n]])){
    if (grepl('athletics', training_index[[n]][i])) {
      training_index[[n]][i] <- 1
    } else if (grepl('cricket', training_index[[n]][i])) {
      training_index[[n]][i] <- 2
    } else if (grepl('football', training_index[[n]][i])) {
      training_index[[n]][i] <- 3
    } else if (grepl('rugby', training_index[[n]][i])) {
      training_index[[n]][i] <- 4
    } else if (grepl('tennis', training_index[[n]][i])) {
  ...
  for (i in 1:length(testing_index[[n]])){
    if (grepl('athletics', testing_index[[n]][i])) {
      testing_index[[n]][i] <- 1
    } else if (grepl('cricket', testing_index[[n]][i])) {
      testing_index[[n]][i] <- 2
    } else if (grepl('football', testing_index[[n]][i])) {
      testing_index[[n]][i] <- 3
    } else if (grepl('rugby', testing_index[[n]][i])) {
      testing_index[[n]][i] <- 4
    } else if (grepl('tennis', testing_index[[n]][i])) {
      testing_index[[n]][i] <- 5
    }
  }
...
save(training_dtm, testing_dtm, training_index, testing_index, file = "C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\BBCSport data 5 fold in\\created data\\created matrix\\rawdtmBBCsport.Rdata")
