#For Figure 9 of section 6.2 of the first paper: this code is about creating raw document-term matrix for songs from six authors and creating the vector about the Wilhelmus. Note the Wilhelmus is not included in the matrix.
...
author_heere.vector <- rep(c("he"), each = author_len[3])
author_haecht.vector <- rep(c("ha"), each = author_len[4])
author_fruytiers.vector <- rep(c("fr"), each = author_len[5])
author_coornhert.vector <- rep(c("co"), each = author_len[6])
view.vector <- (c(author_datheen.vector, author_marnix.vector, author_heere.vector, author_haecht.vector, author_fruytiers.vector, author_coornhert.vector))
...
save(training_dtm, testing_dtm, author_datheen.vector, author_marnix.vector,author_heere.vector, author_haecht.vector, author_fruytiers.vector, author_coornhert.vector, file ="C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\figure\\Wilhelmus visualization\\sixauthorsLSARAWLSANROWLSATFIDFCA\\rawmatrixsixauthors.Rdata")
