#For Figure 9 of section 6.2 of the first paper: this code is about LSA-NROWL1.
...
td.mat_mfi_choose.total.nrow <- training_dtm/matrix(rep(apply(training_dtm,1,sum),each = ncol(training_dtm)), ncol = ncol(training_dtm), by = TRUE)
testing_dtm_nrow <- testing_dtm/sum(testing_dtm)
...
dev.off()
setEPS()
postscript("C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\figure\\Wilhelmus visualization\\sixauthorsLSARAWLSANROWLSATFIDFCA\\FfoldinsixauthorspredictwilhelmusLSANROWL1.eps")
par(mar=c(5,6,4,1)+.1)
...
plot(td.mat_mfi_choose.total.svd.ud[1:length(author_datheen.vector),1], td.mat_mfi_choose.total.svd.ud[1:length(author_datheen.vector),2], asp = 1, col = "black", cex = 1.8, type = "p", pch = 1, xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd), 
     main = "LSA-NROWL1",
     xlab = "Dimension 1: 2.930 (63.5%)", ylab = "Dimension 2: 0.506 (11.0%)",cex.lab=1.75, cex.axis = 1.55, cex.main=1.75)
