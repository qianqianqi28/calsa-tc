#For Figure 9 of section 6.2 of the first paper: this code is about LSA-TFIDF.
...
dev.off()
setEPS()
postscript("C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\figure\\Wilhelmus visualization\\sixauthorsLSARAWLSANROWLSATFIDFCA\\FfoldinsixauthorspredictwilhelmusLSATFIDF.eps")
par(mar=c(5,6,4,1)+.1)
plot(td.mat_mfi_choose.total.svd.ud[1:length(author_datheen.vector),1], td.mat_mfi_choose.total.svd.ud[1:length(author_datheen.vector),2], asp = 1, col = "black", cex = 1.8, type = "p", pch = 1, xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd), 
     main = "LSA-TFIDF",
     xlab = "Dimension 1: 567,484.200 (48.6%)", ylab = "Dimension 2: 123,019.700 (10.5%)",cex.lab=1.75, cex.axis = 1.55, cex.main=1.75)
...
legend("bottomright",cex=1,c("Datheen","Marnix","Heere","Haecht","Fruytiers","Coornhert"),col=c("black","blue","orange","green", "Violet", "cyan"),pch = c(1, 3, 23, 0, 8, 2))
dev.off()
