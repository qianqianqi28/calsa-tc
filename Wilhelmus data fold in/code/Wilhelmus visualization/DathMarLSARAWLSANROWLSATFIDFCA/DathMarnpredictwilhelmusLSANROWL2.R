#For Figure 7 of section 6.2 of the first paper: this code is about LSA-NROWL2.
...
dev.off()
setEPS()
postscript("C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\figure\\Wilhelmus visualization\\DathMarLSARAWLSANROWLSATFIDFCA\\FfoldinDathMarnpredictwilhelmusLSANROWL2.eps")
par(mar=c(5,6,4,1)+.1)
plot(td.mat_mfi_choose.total.svd.ud[1:length(author_datheen.vector),1], td.mat_mfi_choose.total.svd.ud[1:length(author_datheen.vector),2], asp = 1, col = "black", cex = 1.8, type = "p", pch = 1, xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd), 
     main = "LSA-NROWL2",
     xlab = "Dimension 1: 54.916 (67.8%)", ylab = "Dimension 2: 9.545 (11.8%)",cex.lab=1.75, cex.axis = 1.55, cex.main=1.75)
