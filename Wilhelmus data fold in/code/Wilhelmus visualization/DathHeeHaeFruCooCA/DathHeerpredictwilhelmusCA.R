#For Figure 8 of section 6.2 of the first paper: this code is about Heere.
...
load("C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\figure\\Wilhelmus visualization\\DathHeeHaeFruCooCA\\rawmatrixdathheer.Rdata")
...
dev.off()
setEPS()
postscript("C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\figure\\Wilhelmus visualization\\DathHeeHaeFruCooCA\\FfoldinDathHeerpredictwilhelmusCA.eps")
par(mar=c(5,6,4,1)+.1)
...
plot(td.mat_mfi_choose.total.svd.ud[1:length(author_datheen.vector),1], td.mat_mfi_choose.total.svd.ud[1:length(author_datheen.vector),2], asp = 1, col = "black", cex = 1.8, type = "p", pch = 1, xlim = c(x.min.udvd,x.max.udvd), ylim = c(y.min.udvd,y.max.udvd), 
     main = "CA",
     xlab = "Dimension 1: 0.143 (11.2%)", ylab = "Dimension 2: 0.115 (9.0%)",cex.lab=1.75, cex.axis = 1.55, cex.main=1.75)
...
legend("topright", cex = 1.75,c("Datheen","Heere"),col=c("black","blue"),pch = c(1, 3))
dev.off()
