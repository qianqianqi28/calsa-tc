#For section 6.3 of the first paper: this code is to calculate accuracy about CA
...
dimdefi <- rankMatrix(training_dtm.SR)
...
for (k in 1:dimdefi){
  ...
  for(i in loo.len){
    ...
    dtm.training.caraw.svd <- dtm.training.caraw.svd.list[[i]]
    training_dtm.csc <- training_dtm.Dcmh[[i]]%*%dtm.training.caraw.svd$v
    
    #CARAW
    td.mat_mfi_choose.svd.ud <- as.matrix(as.matrix(training_dtm.Drmh[[i]]%*%as.matrix(dtm.training.caraw.svd$u)%*%as.matrix(diag((dtm.training.caraw.svd$d))))[,1:k])
    
    temp <- testing_dtm[[i]]/sum(testing_dtm[[i]])
  ...
  average.averagelinkage.accuracy[k]
  average.singlelinkage.accuracy[k]
  average.completelinkage.accuracy[k]
  print(average.centroidlinkage.accuracy[k])
}

caaveragelinkage <- average.averagelinkage.accuracy
write.csv(caaveragelinkage, file ="C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\created data\\average linkage\\caaveragelinkage.csv")

casinglelinkage <- average.singlelinkage.accuracy
write.csv(casinglelinkage, file ="C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\created data\\single linkage\\casinglelinkage.csv")

cacompletelinkage <- average.completelinkage.accuracy
write.csv(cacompletelinkage, file ="C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\created data\\complete linkage\\cacompletelinkage.csv")

cacentroidlinkage <- average.centroidlinkage.accuracy
write.csv(cacentroidlinkage, file ="C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 1\\20220417 Review 2 archive\\Wilhelmus data fold in\\created data\\centroid linkage\\cacentroidlinkage.csv")
